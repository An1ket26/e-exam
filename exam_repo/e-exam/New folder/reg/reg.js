...
register.onclick = () => {
  ...
  // console.log(regex.test(fname.value));
  if (fname.value.trim().length < 5 || regex.test(fname.value.trim()) ===false) {
    var e1 = document.getElementById("e1");
    e1.classList.remove("err1");
    e1.classList.add("err");
    returnval = false;
  }
  regex = /^[0-9]{2}[a-zA-z]{4}[0-9]{2}$/;
  if (sid.value.trim().length <8 || regex.test(sid.value.trim()) ===false) {
    var e2 = document.getElementById("e2");
  ...
  if (
    regex.test(email.value.trim()) ===false
